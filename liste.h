...
/* le struct sono state definite dal professor Patrignani durante le lezioni di algoritmi e strutture dati 
 * un modo alternativo con cui ad oggi scrivo le struct é:
 * typedef struct l{
    int info;
    struct l* next;
  }lista;
  
  typedef lista* punta_lista;
  */
...
int verifica_ultimo_maggiore_primo(punta_head* l);
